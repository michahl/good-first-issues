...
const Issues = ({ checked }) => {
    ...
    useEffect(() => {
        const fetchSavedIssues = async () => {
            ...
            let arr = []
            saved.forEach(i => arr.push(helper.extractIssueDetails(i)))
    
            const fetchPromises = arr.map(issue => 
                helper.fetchIssueDetails(issue.owner, issue.repo, issue.issueNumber)
            );
    ...
    const toggleSaveIssue = (issueId) => {
        const newSavedIssues = new Set(savedIssues);
        if (newSavedIssues.has(issueId)) {
            newSavedIssues.delete(issueId);
        } else {
            newSavedIssues.add(issueId);
        }
        const updatedSavedIssues = [...newSavedIssues];
  ...
  return (
    <div className='my-2'>
        <div>
            <div className='h-[480px] md:h-[550px] overflow-y-auto'>
                {(checked ?savedData : issues).map(issue => (
                    <div className='border rounded-xl mb-4 relative hover:shadow-lg transition-shadow ease-in-out duration-200 dark:border-zinc-600' key={issue.id}>
                        <div className='w-full rounded-t-xl relative'>
                            <span onClick={() => toggleSaveIssue(issue.id)} className='flex justify-end items-center px-3 py-1 absolute right-1 top-2 cursor-pointer text-zinc-500'>
                                {!savedIssues.includes(issue.id) ? (
                                    <svg className='w-5 h-5' viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <div className='px-4 py-3.5'>
                            ...
                            <p className='text-sm text-zinc-500 dark:text-zinc-400 mb-3'>{issue.repository_name}</p>
                            <div className='flex flex-wrap items-center text-sm text-slate-500 dark:text-zinc-500'>
